...
final class SyncEngine {
    ...
    init(defaults: UserDefaults, initialRecipes: [Session]) {
        self.defaults = defaults
        self.buffer = initialRecipes
    ...
    private func uploadLocalDataNotUploadedYet() {
        ...
        let recipes = buffer.filter({ $0.ckData == nil })

        guard !recipes.isEmpty else { return }

        os_log("Found %d local recipe(s) which haven't been uploaded yet.", log: self.log, type: .debug, recipes.count)

        let records = recipes.map { $0.record }

        uploadRecords(records)
    }
    
    func upload(_ recipe: Session) {
        os_log("%{public}@", log: log, type: .debug, #function)

        buffer.append(recipe)

        uploadRecords([recipe.record])
    }

    func delete(_ recipe: Session) {
